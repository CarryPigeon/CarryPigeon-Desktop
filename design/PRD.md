...
> 说明：本文件归档在 `design/` 目录下，作为项目设计文档的入口之一。
...
- 本 PRD 不包含：具体 UI 视觉稿、完整接口细节与字段枚举（详见 `design/protocol/*`）、具体实现设计（详见 `design/client/*`、`design/plugin/*` 与 ADR）。
...
补充规范：
- 插件中心/安装向导的完整状态机与错误处理见：`design/client/PLUGIN-CENTER-FLOWS.md`。
