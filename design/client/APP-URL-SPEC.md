...
- Scheme：`app://`
- 插件资源根：`app://plugins/`
- URL 作为“资源定位符”，应与本地安装目录一一对应（见 `design/client/PLUGIN-PACKAGE-STRUCTURE.md`）。
...
其中：
- `<server_id>`：服务端返回的固定 UUID（见 `design/PRD.md`、`design/protocol/PROTOCOL-OVERVIEW.md`）
...
宿主可提供 `getAssetUrl("assets/icon.png")`，返回对应 `app://...` URL（见 `design/client/PLUGIN-PACKAGE-STRUCTURE.md`）。
