...
#[tauri::command]
pub async fn save_emoji(app_handle: AppHandle, source_path: String, name: String, tags: Vec<String>) -> CommandResult<EmojiEntry> {
    let id = uuid::Uuid::new_v4().to_string();
    let entry = repository::add_emoji(&app_handle, &id, &name, std::path::Path::new(&source_path), &tags)?;
...
pub async fn delete_emoji(app_handle: AppHandle, id: String) -> CommandResult<()> {
    repository::delete_emoji(&app_handle, &id)?;
...
pub async fn get_emoji_image_path(app_handle: AppHandle, id: String) -> CommandResult<String> {
    let index = repository::load_index(&app_handle);
    let entry = index.items.iter().find(|e| e.id == id).ok_or("emoji not found")?;
    let full_path = app_handle.path().app_data_dir().unwrap_or_default()
        .join("custom-emoji")
