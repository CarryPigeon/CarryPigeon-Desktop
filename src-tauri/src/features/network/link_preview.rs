...
pub async fn fetch_link_preview(url: String) -> CommandResult<LinkPreviewDto> {
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| to_command_error("LINK_PREVIEW_CLIENT_BUILD_FAILED", e))?;

    let resp = client
        .get(&url)
        .send()
        .await
        .map_err(|e| to_command_error("LINK_PREVIEW_FETCH_FAILED", e))?;
    ...
    let bytes = resp
        .bytes()
        .await
        .map_err(|e| to_command_error("LINK_PREVIEW_READ_BODY_FAILED", e))?;
