...
/// 下载进度回调函数类型定义
type ProgressCallback = dyn Fn(u64, u64) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send>> + Send + Sync + 'static;
...
pub async fn download_avatar_impl(
    ...
    let content_length = match client.head(url).send().await {
        Ok(resp) => resp.headers().get(reqwest::header::CONTENT_LENGTH)
    ...
    info!("文件下载完成: {}, 大小: {} 字节, SHA256: {}", 
          output_path.display(), downloaded, hash);
