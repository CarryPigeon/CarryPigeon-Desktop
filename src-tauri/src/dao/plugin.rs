    ...
    for<'r> T: sqlx::FromRow<'r, sqlx::sqlite::SqliteRow> + std::marker::Unpin + std::marker::Send,
{
    ...
    Box::new(sqlx::query_as(&sql)
        .fetch_all(SQLITE_POOL.get().unwrap())
        .await)
}

pub async fn create_table(plugin_name: String, table_name: String, columns: String) -> Box<Result<SqliteQueryResult, sqlx::Error>>{
    let sql = format!("create table {plugin_name}_{table_name} ({columns})");
    Box::new(sqlx::query(&sql)
        .execute(SQLITE_POOL.get().unwrap())
        .await)
}