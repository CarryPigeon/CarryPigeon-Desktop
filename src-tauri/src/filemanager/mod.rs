...
pub fn file_path_distributor(file_name: String, file_type: String) -> anyhow::Result<String>{
    ...
    match FileType::from_str(file_type.as_str()){
        Ok(file_type) => match file_type {
            FileType::Image => {
                // TODO: 实现图片文件路径分发逻辑
                anyhow::Ok(Path::new(format!("./Image/{}/{}",today,file_name).as_str()).to_str().unwrap().to_string())
            },
            FileType::Video => {
                anyhow::Ok(Path::new(format!("./Video/{}/{}",today,file_name).as_str()).to_str().unwrap().to_string())
            },
            FileType::Audio => {
                anyhow::Ok(Path::new(format!("./Audio/{}/{}",today,file_name).as_str()).to_str().unwrap().to_string())
            },
            FileType::Document => {
                anyhow::Ok(Path::new(format!("./Document/{}/{}",today,file_name).as_str()).to_str().unwrap().to_string())
            },
            FileType::Other => {
                anyhow::Ok(Path::new(format!("./Other/{}/{}",today,file_name).as_str()).to_str().unwrap().to_string())
            }
        },
        Err(err) => {
            tracing::error!("{:?}",err);
