...
- 日志不要拼接长字符串，优先用结构化 `meta`：
  - ✅ `logger.error("Action: handshake_failed", { serverSocket, error: String(e) })`
  - ❌ `logger.error("Action: handshake_failed: " + e)`
- 不在日志中输出敏感信息（token、密钥、邮箱验证码、私钥等）。

### 1.2.1 强制写法（跨端统一检索 + 二次分层）

为了让前端与 Rust 日志统一检索与聚合，以下写法为强制规范：

- **前端（createLogger / tauriLog）**：`message` 必须为 `Action: <snake_case>`，且必须带二次分层前缀（如 `chat_` / `network_` / `plugins_`），并把关键字段放到 `meta` 中：
  - ✅ `logger.warn("Action: http_client_request_failed", { transport, method, url, status, reason })`
- **Rust（tracing）**：必须包含结构化字段 `action="..."`，且必须带二次分层前缀（如 `app_` / `network_` / `settings_`），并把关键字段以 `key = %value` / `key = ?value` 的形式写入：
  - ✅ `tracing::warn!(action = "network_tcp_read_failed", error = ?e);`

### 1.2.2 自动检查（守门）

- 前端日志规范检查：`bash scripts/check-log-standards.sh`
- Rust 日志规范检查：`bash scripts/check-rust-standards.sh`
- 二次分层词汇表：`docs/日志Action词汇表.md`
