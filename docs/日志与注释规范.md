...
为了让前端与 Rust 日志更容易统一检索与聚合，建议：

- **前端（createLogger）**：使用稳定的动作标识作为 message（或以 `Action:` 前缀开头），并把关键字段放到 `meta` 中：
  - ✅ `logger.warn("Action: http_request_failed", { transport, method, url, status, reason })`
- **Rust（tracing）**：优先使用结构化字段 `action="..."`，并把关键字段以 `key = %value` / `key = ?value` 的形式写入：
