...
为了对齐 UI 规格中的阶段表达（`docs/ui/方案A-插线板/04-页面规格-登录与连接.md`），建议客户端把连接流程拆为：
...
- `GET /api/server`（见 `docs/api/11-HTTP端点清单（v1，标准版）.md`）
...
详见 `docs/api/13-错误模型与Reason枚举（v1，标准版）.md`。
...
事件细节见 `docs/api/12-WebSocket事件清单（v1，标准版）.md`。
