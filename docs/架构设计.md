...
- `src/features/<feature>/`
  ...
  - `composition/`：组装入口（实现选择、对象装配、生命周期缓存）
  - `mock/`、`test/`：按需
...
- feature 内：`presentation -> composition -> domain <- data`。
- `domain` 禁止依赖 `data/presentation/di`、Vue、Tauri、浏览器 API。
- 跨 feature 禁止直连内部实现，仅通过 `@/features/<feature>/api` 或 `shared`。
- `app/` 作为应用组装层，若需要 feature 级公共资源，必须通过显式公共入口（如 `@/features/<feature>/styles`），不能直连 feature 内部目录。
...
- 边界约束通过 `scripts/check-feature-boundaries.sh` 做静态扫描，作为模块化架构的 fitness function。
