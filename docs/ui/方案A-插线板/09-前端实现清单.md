...
1) 主题切换与 Token 落地
- 修改：`src/App.vue`（新增 `:root[data-theme="patchbay"]` Token，参考 `docs/ui/方案A-插线板/14-样式落地参考.md`）
...
- 数据模型：
  - 调整消息实体（建议）：`docs/ui/方案A-插线板/13-数据模型与契约.md`
...
- 插件 composer 对接：
  - 参考 `design/plugin/PLUGIN-COMPOSER-UI.md`（submit/cancel/dirty-change）
...
- 新增 store（建议）：`src/features/network/presentation/store/connectionStore.ts`
  - 暴露：`state/reason/detail`（见 `docs/ui/方案A-插线板/11-状态机与错误码映射.md`）
...
- 参考清单：`docs/ui/方案A-插线板/12-i18n与文案清单.md`
