...
## 2. TCP/Netty 连接与传输安全（TLS）

### 2.1 TCP 端口

- TCP 协议：支持纯 TCP 与 TLS（非 HTTP）
...
### 2.3 传输安全（TLS，用于传递 AES 会话密钥）

本项目抛弃“ECC 公钥加密 AES key”的做法，改为使用 **TLS 先建立安全信道**，然后在 TLS 信道内发送 AES 会话密钥包（握手仍沿用 `route="handshake"` 的确认机制）：

- TLS 负责：机密性 + 完整性 + 服务端身份校验（证书链/主机名）。
- AES-GCM（应用层）仍负责：业务 payload 的加密与重放保护（AAD：sequence/session_id/timestamp）。

证书建议：
- 推荐：服务端使用可信 CA 证书（例如 Let’s Encrypt），客户端 Strict 校验。
- 自建/内网：可采用指纹 Pinning（按 `server_socket` 固定信任）或企业/自建 CA；不要默认关闭校验。

> 注意：启用 TLS 后，服务端应监听 TLS 端口，并且客户端的 `server_socket` 应指向 TLS 端口（建议使用 `tls://host:port` 表达）。
...
### 2.4 业务包（AES-GCM + AAD，可运行在 TLS 之上）
...
- `payload`：业务 JSON 文本（即第 3 节的 `CPPacket` / `CPResponse`），运行在 TLS 之上
