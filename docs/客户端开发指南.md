...
### 3.3 WebView CSP 现状与已知取舍（重要）

当前 `src-tauri/tauri.conf.json` 的 CSP 中 `connect-src` 为 `'self' http: https: ws: wss: app:`，等效于**不限制 WebView 内的出联目标**。这是为支持"用户可添加任意自托管服务器（含局域网明文 http://）"做出的已知取舍：

- **风险**：若渲染层被注入脚本（XSS / 恶意插件前端），数据可被外传至任意主机，CSP 不会拦截。
- **约定**：
  - 新增前端直连网络能力前必须评估是否可以走 Rust 侧代理命令（如 `api_request_json`、`download_file`），不要默认依赖 CSP 放行；
  - 服务端地址建议优先 `https/wss` 或 `tls-fp://`（证书指纹钉扎），避免明文 `http/ws`；
  - 后续若产品放弃明文自托管场景，应将 `connect-src` 收敛为 `'self' app: https: wss:` 并移除明文 scheme。
