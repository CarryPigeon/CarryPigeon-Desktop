...
- 下载来源必须是已声明且受信任的 catalog/source，客户端不得把未经允许的跨源下载地址当作插件包。
- 这里说的受信任来源，默认指经过项目 CI 质量门禁验证过的发布产物，至少应覆盖 lint、build、test、audit 这些阶段。
...
- 更新后的解压和启用也要复用同样的路径 containment 检查，防止 symlink 或穿越条目污染旧版本目录。
