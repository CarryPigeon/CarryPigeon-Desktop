...
function parseSmallInteger(value: unknown): number | null {
...
function parseHandshakeEnvelope(value: unknown): HandshakeEnvelope | null {
    if (!isJsonObject(value)) return null;
    const id = parseSmallInteger(value["id"]);
    const code = parseSmallInteger(value["code"]);
    if (id !== -1 || code !== 0) return null;

    const data = value["data"];
    if (!isJsonObject(data)) return null;
    if (String(data["route"] ?? "").trim() !== "handshake") return null;

    const inner = data["data"];
    if (!isJsonObject(inner)) return null;
    return { sessionIdRaw: inner["session_id"] };
}

function extractSessionIdDigitsFromText(text: string): string | null {
    const match = /"session_id"\s*:\s*(?:"(\d+)"|(\d+))/.exec(text);
    const digits = (match?.[1] ?? match?.[2] ?? "").trim();
    return digits || null;
}

function parseSessionIdFromUnknown(value: unknown): bigint | null {
    if (typeof value === "number") {
        ...
        if (numeric <= 0n) throw new Error(`Invalid session_id: ${value}`);
        return numeric;
    }
...
export class Encryption {
    ...
    public tryHandleHandshakeResponseText(text: string): boolean {
        ...
        try {
            ...
            const envelope = parseHandshakeEnvelope(parsed);
            if (!envelope) return false;

            const digits = extractSessionIdDigitsFromText(text);
            if (!digits) throw new Error("Missing session_id in handshake payload");

            const sessionId = BigInt(digits);
            if (sessionId <= 0n) throw new Error(`Invalid session_id: ${digits}`);

            this.setHandshakeSession(sessionId);
            return true;
        } catch (e) {
    ...
    public tryHandleHandshakeResponse(value: unknown): boolean {
        const envelope = parseHandshakeEnvelope(value);
        if (!envelope) return false;

        try {
            const sessionId = parseSessionIdFromUnknown(envelope.sessionIdRaw);
