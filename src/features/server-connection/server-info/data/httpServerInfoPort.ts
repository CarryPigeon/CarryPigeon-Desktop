 ...
 * @description server-connection/server-info｜数据层实现：httpServerInfoPort。
 *
 * API 文档：
...
import { ServerInfoError, toServerInfoError } from "../domain/errors/ServerInfoError";
...
export const httpServerInfoPort: ServerInfoPort = {
  async getServerInfo(serverSocket: string): Promise<ServerInfo> {
    ...
    try {
      ...
      const serverInfoError = toServerInfoError(
        "get_server_info_failed",
        "Get server info failed.",
        error,
        { serverSocket: socket },
      );
      logger.error("Action: servers_info_get_failed", { socket, error: serverInfoError.message, code: serverInfoError.code });
      throw serverInfoError;
    }
  },
};
