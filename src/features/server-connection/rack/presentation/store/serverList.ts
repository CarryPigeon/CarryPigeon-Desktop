...
type StoredServersState = StoredServerRacksState;

/**
 * 生成 rack id（优先使用 `crypto.randomUUID`）。
 *
 * @returns 稳定唯一的 rack id。
 */
function createRackId(): string {
  if (typeof globalThis.crypto?.randomUUID === "function") return `rack-${globalThis.crypto.randomUUID()}`;
...
function normalizeRack(raw: Partial<ServerRack> & { id?: unknown; serverSocket?: unknown; name?: unknown }): ServerRack {
  const id = typeof raw.id === "string" && raw.id.trim() ? raw.id : createRackId();
...
const state = ref<StoredServersState>(
  getServerRackStatePort().read(),
);
let rackRuntimeStarted = false;
 ...
 * @returns 当前机架列表。
 */
function computeServerRacks(): ReadonlyArray<ServerRack> {
  return state.value.servers.map((rack) => ({ ...rack }));
}
 ...
 * @constant
 */
export const serverRacks = computed<ReadonlyArray<ServerRack>>(computeServerRacks);
 ...
 *
 * 说明：
 * - 显式注册 TLS provider，避免模块加载时产生副作用。
 */
export function startServerRackRuntime(): void {
  if (rackRuntimeStarted) return;
  rackRuntimeStarted = true;
  // 依赖倒置：由 server-connection/rack feature 提供 per-server TLS 配置，供 shared/net 基础设施消费。
  setServerTlsConfigProvider(getTlsConfigForSocket);
}

/**
 * 停止 rack 运行时（best-effort）。
 *
 * 说明：
 * - 回退 TLS provider 为默认 strict，避免上游拿到脏引用。
 */
export function stopServerRackRuntime(): void {
  if (!rackRuntimeStarted) return;
  rackRuntimeStarted = false;
  setServerTlsConfigProvider(() => ({ tlsPolicy: "strict", tlsFingerprint: "" }));
}
