 ...
 * @description chat/room-session｜application：resume.failed 后的会话恢复补拉策略。
 *
 * 背景：
...
export function createResumeFailedCatchUp(deps: ResumeFailedCatchUpDeps) {
  ...
  return async function catchUpAfterResumeFailed(socketKey: string, reason: string): Promise<void> {
    ...
    const isStale = (): boolean =>
      deps.getActiveServerSocket() !== key || deps.getActiveScopeVersion() !== requestScopeVersion;
    if (isStale()) return;
    ...
    if (current && !isStale()) void deps.refreshMembersRail(current);
  };
}
