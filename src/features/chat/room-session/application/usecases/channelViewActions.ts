...
export function createChannelViewActions(deps: ChannelViewActionsDeps) {
  ...
  function getMessageById(channelId: string, messageId: string): ChatMessage | null {
    return deps.state.findMessageById(channelId, messageId);
  }
  ...
  async function selectChannel(id: string): Promise<ChannelSelectionOutcome> {
    ...
    const ch = deps.state.findChannelById(cid);
    if (!ch) {
      return {
        ok: false,
        kind: "chat_channel_selection_rejected",
    ...
    deps.state.setCurrentChannelId(cid);
    const prevUnread = deps.state.readUnreadCount(cid);
    // 目录上的未读角标在“进入频道”时先做本地乐观清零；若加载失败则回滚。
    deps.state.markChannelReadLocally(cid);

    try {
      await deps.loadChannelMessages(cid);
    } catch (e) {
      if (prevUnread > 0) deps.state.incrementChannelUnread(cid, prevUnread);
      return {
        ok: false,
        kind: "chat_channel_selection_rejected",
    ...
    const list = deps.state.listMessages(cid);
    ...
    // 先推进本地单调读时间，再根据节流策略决定是否向远端上报。
    const nextReadTime = deps.readStateReporter.advanceLocalReadTime(cid, now);
    deps.state.writeLastReadMessageId(cid, lastMid);
    const lastReportAt = deps.state.readLastReportAtMs(cid);
    const shouldReport =
  ...
  async function reportCurrentReadState(): Promise<void> {
    const cid = deps.state.readCurrentChannelId();
    if (!cid) return;
    const list = deps.state.listMessages(cid);
    ...
    const now = Date.now();
    // 当前动作只表达“我已看到这条消息”，实际节流/去抖规则仍由 reporter 持有。
    const nextReadTime = deps.readStateReporter.advanceLocalReadTime(cid, now);
    deps.state.writeLastReadMessageId(cid, lastMid);
    void deps.readStateReporter.reportIfAllowed(cid, lastMid, nextReadTime, now, 1500);
  }
