 ...
 * @description chat/room-session｜application：WS 不可用时的 HTTP polling 降级控制器。
 *
 * 背景：
...
export type PollingFallbackDeps = {
  /**
   * polling 间隔（毫秒）。
   */
  intervalMs: number;
 ...
 */
export type PollingFallbackController = {
  /**
 ...
 * @returns controller。
 */
export function createPollingFallback(deps: PollingFallbackDeps): PollingFallbackController {
  let pollTimer: ReturnType<typeof setInterval> | null = null;
  let pollKey: string = "";
  let pollScopeVersion: number = -1;
  ...
  function start(socketKey: string): void {
    ...
    pollKey = key;
    pollScopeVersion = deps.getActiveScopeVersion();
      ...
      void (async () => {
        try {
          const isStale = (): boolean =>
            deps.getActiveServerSocket() !== key || deps.getActiveScopeVersion() !== pollScopeVersion;
          if (isStale()) return;
