...
- 跨 feature 仅通过 `src/features/chat/api.ts` 使用 chat 能力；主入口为 `createChatCapabilities()` / `getChatCapabilities()`（object-capability）。
- `ChatCapabilities` 当前分为 `session` / `messageFlow` / `governance` 三组最小命令/查询能力；根 capability 不暴露 `use*View()` 这类 Vue 视图对象。
- `src/features/chat/api-types.ts` 仅导出稳定公共类型；聚合 runtime store / runtime slice 类型不属于跨 feature 契约。
- `chat/api.ts` 是 chat 跨 feature 唯一值入口，详见 `docs/Feature API公开约束.md`。
- `src/features/chat/routes.ts` 是 chat 页面路由的唯一公共入口；`app/router` 只应通过该入口装配聊天页面。
- `chat/typechecks/*`、`chat/domain/ports/*`、`chat/data/*`、`chat/room-session/*`、`chat/message-flow/*`、`chat/room-governance/*`、`chat/presentation/*` 视为 chat 内部实现，不作为跨 feature 稳定入口。
- `src/features/chat/styles.ts` 是 app composition root 唯一允许消费的公共样式入口；`presentation/styles/*` 仍属于内部实现。

## 关键概念

- **ChatApiPort / ChatEventsPort**：聊天 HTTP 与事件（WS）端口抽象（domain 层）。
- **chat composition root**：`di/chatRuntime.ts` 是 chat 唯一的对象生命周期归口；ports、usecases、runtime gateway 与聚合 runtime 都在这里集中装配与缓存。
- **chat usecase factory**：`di/chatUsecases.ts` 只描述显式用例的创建方式，不承担全局缓存职责。
- **ChannelSummary**：`contracts/channelSummary.ts` 是 room-session / room-governance 共享的中立频道摘要契约，避免子域互相牵引。
...
- **chat runtime contracts**：`presentation/store/live/chatRuntimePorts.ts` 统一转发 `chatSessionRuntimePorts.ts`、`chatMessageFlowRuntimePorts.ts`、`chatGovernanceRuntimePorts.ts` 与 `chatScopePort.ts`，避免局部运行时契约继续挤在单文件里。
- **chat session shared context**：`presentation/store/live/chatSessionSharedContext.ts` 统一封装 socket/token/scope、频道目录刷新与读状态上报，作为各 runtime 共享的会话上下文适配层。
- **session connection runtime**：`presentation/store/live/chatSessionConnectionRuntime.ts` 只负责 room-session 中的连接生命周期；外层 `presentation/store/live/chatSessionRuntime.ts` 负责把连接子系统与频道视图动作聚合为公开会话能力。
- **domain 渲染**：
...
- `data/`：HTTP/WS 适配器实现（以及 mock/protocol mock 的接入点）。
  - `data/wire/*`：集中承载协议层 snake_case wire contract；`data/*Port.ts` 负责把它们映射到 `domain/types/*` 的 camelCase 领域契约。
- `typechecks/`：仅用于 TypeScript 编译期契约检查，不承载运行时实现或稳定公共契约。
- `contracts/`：chat 子域间共享的中立契约。
- `di/`：依赖装配（ports / usecases / runtime 分层）。
  - `chatPorts.ts`：底层 ports 构造。
  - `chatUsecases.ts`：显式用例工厂。
  - `chatRuntime.ts`：聚合 runtime 与子域公开 store 的唯一 composition root。
  - `chatGatewayFactory.ts`：组装展示层消费的 runtime API/events gateway。
- `room-session/`：会话上下文与连接阶段编排。
- `room-session/contracts.ts`：频道模型，由 room-session 自己持有。
- `room-session/application/`：会话就绪、连接、补拉、读状态与切服重置等非 UI 应用编排。
- `message-flow/`：消息列表、消息语义、发送与渲染扩展点。
- `message-flow/application/`：消息动作、分页、domain 列表、事件路由与消息模型归一化等非 UI 应用编排。
- `message-flow/contracts.ts`：消息模型与 composer payload，由 message-flow 自己持有。
- `room-governance/`：成员/申请/禁言/频道治理能力。
- `room-governance/contracts.ts`：治理相关成员/申请/封禁模型，由 room-governance 自己持有。
- `room-governance/application/`：治理动作编排、DTO 映射与 scope guard；承载非 UI 的频道治理应用逻辑。
- `integration/`：chat 对 account / server-connection / plugins 的适配层；chat 内部子模块优先依赖这里，而不是直接跨 feature import。
  - `integration/pluginAccess.ts`：只暴露 chat 真正需要的插件目录摘要与 required-gate 摘要，不把 plugins workspace 全量语义带进 chat 页面。
  - `integration/pluginRuntime.ts`：只负责消息 domain、runtime view 与 host bridge 相关能力。
- `presentation/`：页面、组件、composables、store（UI 与状态）。
  ...
  - `store/live/chatSessionSharedContext.ts`：chat runtime 共享的会话上下文，隔离 scope/token/channel-data/read-state 等跨子域基础设施。
  - `store/live/chatSessionConnectionRuntime.ts`：session 子系统中的连接生命周期运行时，隔离 WS/polling/catch-up 等连接细节。
  ...
  - `application/createChatWorkspaceCoordinator`：收敛当前 workspace 的启动、切服、依赖刷新与插件桥接链路。
  - `composables/usePatchbayWorkspace`：收敛 workspace 级读模型与跨 feature 协调（server/plugin/host-bridge），不承担完整页面装配。
  - `composables/usePatchbayPageModel`：作为 MainPage 的页面 composition root，负责把 workspace、子域 capability、导航、浮层、快捷键、生命周期和 layout model 组装成最终页面模型。
  - `composables/patchbayPageSections`：承载 Patchbay 各局部 section 的局部模型类型与构造器，避免页面 composition root 再次膨胀成样板聚合点。
...
- room-session 会话编排子模块：`src/features/chat/room-session/presentation/store/`
- message-flow 消息流子模块：`src/features/chat/message-flow/presentation/store/`
...
- room-governance 治理子模块：`src/features/chat/room-governance/presentation/store/`
- 领域 ports：
