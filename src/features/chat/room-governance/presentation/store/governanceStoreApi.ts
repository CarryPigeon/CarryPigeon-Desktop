/**
 * @fileoverview room-governance 稳定公开 Store API。
 * @description
 * 对外暴露 room-governance 子域能力。
 * 该文件只转发 runtime 管理的当前子域 store，不暴露聚合 store 或内部运行时实现。
 */

import { computed } from "vue";
import { getRoomGovernanceStore } from "@/features/chat/di/chat.di";
import type {
...
import type { RoomGovernanceRuntimeStore } from "@/features/chat/presentation/store/chatStoreTypes";

/**
 * 延迟解析当前 room-governance store，避免 import 时绑定到过期 runtime。
 */
function resolveRoomGovernanceStore(): RoomGovernanceRuntimeStore {
