...
import { getRoomGovernanceCapabilities, type ChannelBan, type ChannelMember } from "@/features/chat/room-governance/api";
 ...
 * @returns 频道封禁页状态与动作。
 */
export function useChannelBansPage(): ChannelBansPageModel {
  ...
  async function loadData(): Promise<void> {
    await runPageLoad(
      async (channelId) => {
        const channelGovernance = roomGovernanceCapabilities.forChannel(channelId);
  ...
  async function handleAddBan(): Promise<void> {
    ...
    await runChannelAction(
      (channelId) => roomGovernanceCapabilities.forChannel(channelId).setBan(selectedUid.value, until, banReason.value),
      async () => {
        closeAddBanDialog();
        await loadData();
      },
  ...
  async function handleRemoveBan(uid: string): Promise<void> {
    await runChannelAction(
      (channelId) => roomGovernanceCapabilities.forChannel(channelId).removeBan(uid),
      loadData,
    );
  }
