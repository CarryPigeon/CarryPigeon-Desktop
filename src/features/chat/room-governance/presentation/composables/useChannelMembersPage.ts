 ...
 * @returns 成员页状态与动作。
 */
export function useChannelMembersPage(): ChannelMembersPageModel {
  ...
  async function loadMembers(): Promise<void> {
    await runPageLoad((channelId) => roomGovernanceCapabilities.forChannel(channelId).listMembers(), (nextMembers) => {
  ...
  async function handleKick(uid: string): Promise<void> {
    ...
    await runChannelAction(
      (channelId) => roomGovernanceCapabilities.forChannel(channelId).kickMember(uid),
      loadMembers,
    );
  }
  ...
  async function handleSetAdmin(uid: string): Promise<void> {
    await runChannelAction(
      (channelId) => roomGovernanceCapabilities.forChannel(channelId).setAdmin(uid),
      loadMembers,
    );
  }
  ...
  async function handleRemoveAdmin(uid: string): Promise<void> {
    await runChannelAction(
      (channelId) => roomGovernanceCapabilities.forChannel(channelId).removeAdmin(uid),
      loadMembers,
    );
  }
