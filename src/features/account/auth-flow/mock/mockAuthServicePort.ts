 ...
 * @description account/auth-flow｜Mock 实现：mockAuthServicePort（用于本地预览/测试）。
 *
 * required 插件闸门：
...
export function createMockAuthServicePort(serverSocket: string): AuthServicePort {
  return {
    ...
    async tokenLogin(token: string): Promise<TokenLoginResult> {
      await sleep(MOCK_LATENCY_MS);
      if (!token.trim()) throw new AuthError({ code: "missing_token", message: "Missing token." });
      return { accessToken: token, uid: "1" };
    },
    async refreshAccessToken(refreshToken: string): Promise<AuthLoginResult> {
      await sleep(MOCK_LATENCY_MS);
      if (!refreshToken.trim()) throw new AuthError({ code: "missing_refresh_token", message: "Missing refresh token." });
      return {
